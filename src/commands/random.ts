...
import { parseDifficulty, parseStatus } from './list.js';
...
export function registerRandom(program: Command): void {
  program
    ...
    .action(async (opts: { difficulty?: string; tag?: string[]; status?: string; pick?: boolean; lang?: string }) => {
      ...
      const probe = await client.problemList({ difficulty, tags: opts.tag, status, limit: 1, skip: 0 });
      if (probe.total === 0) {
        console.log('No problems match those filters.');
        return;
      }
      const skip = Math.floor(Math.random() * probe.total);
      const { items } = await client.problemList({ difficulty, tags: opts.tag, status, limit: 1, skip });
